...
- **考试**：视频页「考试」页签 + 课程详情「课程试卷」→ 在线答题（**倒计时 + 时间窗口强制 + 到点自动交卷**、交卷幂等）→ 成绩与逐题解析
...
每一轮改动都走同一套节奏：**先定位根因 → 再改 → 然后实测验证**（不满足于"看起来对了"）。开发过程记录保存在本地，不随仓库发布；这里只列做过什么：

| 类型 | 具体问题 |
| --- | --- |
| 口径设计 | 作答与判分链路：交卷时**冻结题目与答案的快照**，判分只读快照 → 之后讲师改题不影响已交卷成绩；答案归一后比较、多选不给部分分、交卷幂等 |
| 权限与安全 | 鉴权加固：修掉一处**试卷答案泄漏**（未交卷也能拿到答案），并把课程归属校验从 Controller 下沉到服务层 |
| 隐蔽缺陷 | 「老师改了、学生端看不到」——根因是 `ON DUPLICATE KEY UPDATE` 的更新列表漏列，**漏掉的字段永远保持旧值且不报错**；连续两轮被误判为前端/缓存问题 |
| 链路重建 | 复活一个**从未成功构建过**的服务（表象是缺云凭证，根因是模块漏依赖）；视频链路本地化时发现并规避网关 Range 转发的损坏 |
| 数据口径 | 学习时长埋点的**防刷设计**（只计播放坐标推进量、单次设上限 → 拖进度条与挂机都不计量）；考试时限取「进入时间 + 时长」与「统一结束时间」的**较早者** |
| 一致性 | 两端对「这一节有没有视频」的判定必须同一口径（只认 `media_id`），避免讲师端说已上传、学生端说没有 |
...
文案统一为「讲师」、清理占位组件、讲师端登录守卫、上架「进行中」反馈。
...
| 7 | 知识库文档编辑端点 | `PUT /ct/file/update` 后端不存在 —— 已先摘掉学员端编辑入口，改为「删除后重新上传」 |
...
| 4 | 老站路由仍挂载 | `legacy.js` 13 条路由指向已被取代的旧页面，其中若干接口后端已不存在 |
| 5 | 无引用的死函数 | `api/*.js` 中约 10 个导出函数无调用点 |
